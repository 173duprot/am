...
#include <stdlib.h>
#include <sys/types.h>
/* #include <sys/socket.h>  No socket access */
#include <sys/un.h>
#include <unistd.h>
...
/* int bmain(int argc, char *argv[]); Bmain is for remote access */
...
/* Noooooooooooooo fuck you sockets
const char *
...
sammain(int argc, char *argv[])
{
    ...
    */
    ...
    /* No socket access for you mister
    if(!dflag)
        startup(machine, rmsocketname != NULL, trylock);
    */
    Fstart(); /* File start, simply opens the buffers */
